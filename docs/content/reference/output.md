...
describes what users get. Pick a format with `-o`, or let epmc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
epmc <command> -o table   # aligned columns for reading
epmc <command> -o jsonl   # one JSON object per line, for piping
epmc <command> -o json    # a single JSON array
epmc <command> -o csv     # spreadsheet friendly
epmc <command> -o tsv     # tab-separated
epmc <command> -o url     # just the URL column
epmc <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
epmc <command> --fields id,title,url
```
...
```bash
epmc <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
epmc <command>            # a table, because this is a terminal
epmc <command> | wc -l    # JSONL, because this is a pipe
```
