// Package cli builds the epmc command tree on top of the europepmc library.
package cli
...
type App struct {
	client *europepmc.Client
	cfg    europepmc.Config
...
func Root() *cobra.Command {
	app := &App{cfg: europepmc.DefaultConfig()}

	root := &cobra.Command{
		Use:   "epmc",
		Short: "Search Europe PMC life sciences literature",
		Long: `epmc reads Europe PMC through its open REST API at ebi.ac.uk.
It covers PubMed (MED), PubMed Central (PMC), and preprints (PPR).
No API key required. Records are emitted as table, JSON, JSONL, CSV, TSV, or URLs.

epmc is an independent tool and is not affiliated with EBI, EMBL, or Wellcome.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = europepmc.NewClient(a.cfg)
	return nil
}
